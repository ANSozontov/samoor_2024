...
tmp <- data.frame(seria = colnames(select(orws, -sp, -Sw))) %>% 
    left_join(distinct(select(labs, seria, coast)), by = "seria") %>% 
    pull(coast)
set.seed(2)
iv.s <- orws %>% 
    rbind(msws) %>%
...
tmp <- data.frame(id = colnames(select(orwi, -sp, -Sw1:-Sw5))) %>% 
...
tables$tab.5_indicator_species <- full_join(
    iv.s, 
    iv.i, 
    by = c("order", "sp", "biotop"))

tables$tab.5_indicator_species %>% 
    DT::datatable(., 
                  filter = 'top', 
...
comm <- list(orws = select(orws, -Sw), 
             orws2 = select(orws, -Sw, -PbTl, -PbPo),
             msws = select(msws, -Sw), 
             msws2= select(msws, -DuCJ, -DuCS, -Sw, -RsFd)) %>% 
    map(~ .x %>% 
...
plot(comm[[1]], main = "Oribatida_all")
plot(comm[[2]], main = "Oribatida_crop")
plot(comm[[3]], main = "Mesostigmata_all")
plot(comm[[4]], main = "Mesostigmata_crop")
dev.off()
