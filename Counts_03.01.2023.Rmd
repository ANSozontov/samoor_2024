...
labs %>% 
    filter(distr == "Samoor") %>% 
    # transmute(id = code, veg, # = factor(veg, ordered = TRUE)
    #           RH, skew, zone, zone.width, substrate, soil) %>% 
    group_by(code) %>% 
    summarise(veg = mean(veg), RH = mean(RH), 
              coast = unique(coast), skew = unique(skew), 
              soil = unique(soil), substrate = unique(substrate), 
              zone = unique(zone), zone.width = mean(zone.width))
```
